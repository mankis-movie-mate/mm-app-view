 ...
 */
export async function getUserWatchlists(username: string, page = 1, size = 10): Promise<PaginatedWatchlists> {
    if (isDev) {
    ...
    const res = await fetchApiWithAuth(`${ACTIVITY_BASE}/watchlist/all-by-user/${username}?page=${page}&size=${size}`);
 ...
 */
export async function createWatchlist(watchlist: Omit<Watchlist, 'id' | 'updated_date'>): Promise<Watchlist> {
    if (isDev) {
