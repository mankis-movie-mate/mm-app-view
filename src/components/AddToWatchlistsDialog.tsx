...
export function AddToWatchlistsDialog({ movieId }: { movieId: string }) {
  ...
  useEffect(() => {
    if (open && data?.elements) {
      setSelected(
        data.elements
          .filter((wl: Watchlist) => wl.movies_id.includes(movieId))
          .map((wl: Watchlist) => wl.id),
      );
    }
  }, [open, data, movieId]);
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      const ops = (data?.elements ?? []).map((wl: Watchlist) => {
        ...
        if (!shouldHave && alreadyHas) {
          return updateWatchlist(wl.id, {
            ...wl,
            movies_id: wl.movies_id.filter((id: string) => id !== movieId),
          });
        }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="bg-indigo-600 hover:bg-indigo-700">+ Add to Watchlist</Button>
      </DialogTrigger>
      <DialogContent className=" bg-gradient-to-b from-indigo-950 via-slate-950 to-black/90 border border-white/15 text-white max-w-md rounded-2xl shadow-2xl max-w-md animate-in shadow-[0_6px_64px_0_rgba(180,60,220,0.16),0_2px_8px_0_rgba(100,50,180,0.11)]">
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {(data?.elements ?? []).length === 0 ? (
              ...
              (data?.elements ?? []).map((wl: Watchlist) => (
                <label
                  key={wl.id}
                  className={`
