...
export default function RecommendPage() {
    ...
    // 1. Search results from backend (paginated)
    const {
        data: searchPage,
        ...
        queryKey: ['movies', 'search', debounced, page, PAGE_SIZE],
        queryFn: () => searchMovies(debounced, { page }),
        enabled: debounced.trim().length > 0,
        staleTime: 30_000,
    });

    // 2. All-movies paginated
    const {
        data: allMovies,
    ...
    const moviesToShow = useMemo(() => {
        if (debounced.trim().length > 0) {
            return searchPage?.elements || [];
        } else {
            return allMovies?.elements || [];
        }
    }, [debounced, searchPage, allMovies]);

    // How many pages total
    const totalPages = useMemo(() => {
        if (debounced.trim().length > 0) return searchPage?.totalPages ?? 1;
        return allMovies?.totalPages ?? 1;
    }, [debounced, searchPage, allMovies]);
                    ...
                    <div>
                        ...
                        {!isLoading && moviesToShow.length > 0 && (
                            <>
                                ...
                                {/* Pagination: always enabled, search is paged! */}
...
/* --- Skeletons, error/info boxes as before --- */
